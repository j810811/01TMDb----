...
class DashboardApp:
    def __init__(self):
        ...
        self.sec_mtime.body.bind("<ButtonRelease-1>", lambda e: self._on_user_split_change("mtime"))
        ...
        self._build_mtime_controls(self.sec_mtime.right)
    ...
    def _load_col_ratios(self):
        ...
        try:
            ...
            for k in list(defaults.keys()):
                ...
                if isinstance(v, (int, float)) and 0.05 <= float(v) <= 0.95:
                    defaults[k] = float(v)
            if defaults["sash1"] <= defaults["sash0"]:
                defaults["sash0"] = 1 / 3
                defaults["sash1"] = 2 / 3
            return defaults
        except Exception:
    ...
    def _apply_splits(self):
        ...
        self.sec_mtime.set_split_ratio(self._split_ratios.get("mtime", 0.75))
    ...
    def _apply_cols(self):
        try:
            ...
            if w <= 1:
                return
            x0 = int(w * self._col_ratios.get("sash0", 1 / 3))
            x1 = int(w * self._col_ratios.get("sash1", 2 / 3))
            if x1 <= x0:
                x0 = int(w * (1 / 3))
                x1 = int(w * (2 / 3))
            self.cols.sash_place(0, x0, 0)
            self.cols.sash_place(1, x1, 0)
        except Exception:
            pass
    ...
    def _on_user_col_change(self):
        try:
            ...
            x1, _y1 = self.cols.sash_coord(1)
            r0 = x0 / w
            r1 = x1 / w
            if r0 < 0.05 or r1 > 0.95 or r1 <= r0:
                return
            self._col_ratios["sash0"] = r0
            self._col_ratios["sash1"] = r1
        except Exception:
            return
